...
class IrActionsReport(models.Model):
    ...
    def _render_qweb_pdf(self, report_ref, res_ids=None, data=None):
        ...
        # ---------------------------------------------------------
            ...
            # Buscamos que tenga la palabra PICK, o nos quedamos con el primero que encontremos
            picking = self.env['stock.picking'].search([
                ('sale_id', '=', order.id),
                ('name', 'ilike', 'PICK')
            ], limit=1)
            
            # Si por alguna razón no se llama PICK, agarramos el primer traslado interno/salida
            if not picking:
            ...
            # Las líneas de los SKUs/Cajas
            moves = picking.move_ids
            total_labels = len(moves)
            
            if total_labels == 0:
                raise UserError(f"El movimiento {picking.name} no tiene líneas de productos.")

            full_zpl_code = ""

            # Recorremos cada línea (caja) del PICK
            for idx, move in enumerate(moves, 1):
                product = move.product_id
                
                # Extracción de campos del SKU (aseguramos que no rompa si el campo viene vacío)
                sku = product.default_code or 'Sin SKU'
                product_name = product.name or 'Sin Nombre'
                # Truncamos el nombre a 40 caracteres para que no se salga de la etiqueta
                ...
                display_name = f"{so_name}/{idx}"
                
                # Código ZPL con la nueva estructura
                zpl_code = f"""
                            ^XA
                            ^CI28
                            ...
                            ^FX 1. Borde exterior de la tabla (700 de ancho, 90 de alto, grosor 3)
                            ^FO50,670^GB700,90,3^FS
                            ^FX 2. Linea divisoria horizontal en medio
                            ^FO50,715^GB700,0,3^FS
                            ^FX 3. Lineas divisorias verticales
                            ^FO225,670^GB0,90,3^FS
                            ^FO400,670^GB0,90,3^FS
                            ^FO575,670^GB0,90,3^FS

                            ^FX 4. Textos de Cabecera (Centrados en sus columnas)
                            ^CF0,25
                            ...
                            ^FX 5. Valores Extraidos (Centrados)
                            ^CF0,30
                            ...
                            ^FX Linea gruesa separadora final
                            ...
                            ^CF0,50
                            ^FO50,1450^FDEtiqueta {idx} de {total_labels}^FS
                            ^XZ
                            """
